...
import { CardDefinition, SinType } from "./gameTypes";
import { PRIDE_CARDS, LUST_CARDS, GLUTTONY_CARDS } from "./newFactions";

// ═══════════════════════════════════════════════════════════════
// WRATH — Aggressive burst damage, self-harm for power
// Passive: OVERCHARGE (burn 2 HP for +1 energy) + SIPHON (heal 20% of compound dmg dealt to others)
...
export const WRATH_CARDS: CardDefinition[] = [
  ...
  },
  {
    ...
    effects: [
      ...
      { type: "heal", baseValue: 2, duration: 3, target: "self" },
    ],
  ...
  },
  {
    ...
    effects: [
      { type: "damage", baseValue: 2, duration: 3, target: "all_enemies" },
      { type: "shield", baseValue: 3, duration: 3, target: "self" },
    ],
...
export const GREED_CARDS: CardDefinition[] = [
  ...
  },
  {
    ...
    effects: [
      { type: "shield", baseValue: 4, duration: 3, target: "self" },
      { type: "heal", baseValue: 3, duration: 3, target: "self" },
    ],
...
// CARD REGISTRY
// ═══════════════════════════════════════════════════════════════
export const ALL_CARDS: CardDefinition[] = [...WRATH_CARDS, ...SLOTH_CARDS, ...GREED_CARDS, ...ENVY_CARDS, ...PRIDE_CARDS, ...LUST_CARDS, ...GLUTTONY_CARDS];
...
export function getDeckForSin(sin: SinType): string[] {
  const cardMap: Record<SinType, CardDefinition[]> = {
    wrath: WRATH_CARDS,
    sloth: SLOTH_CARDS,
    greed: GREED_CARDS,
    envy: ENVY_CARDS,
...
export const NARRATOR_LINES = {
  ...
  siphon: [
    "{player}'s Wrath SIPHONS life from the chaos! Compound damage heals the source. Feeding off pain.",
    "Wrath passive: {player} absorbs 10% of compound damage as healing. Violence IS the answer.",
    "The compound damage ticks and {player} heals from it. Wrath: turning other people's suffering into self-care.",
    "{player} siphons vitality from the carnage. The more chaos, the more they heal. Peak Wrath.",
  ],
  lethargy: [
